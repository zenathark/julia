...
element is returned by [`beginindex(str)`](@ref), and the index of the last element
with [`endindex(str)`](@ref). The keywords `begin` and `end` can be used inside an indexing
...
is not always the same as the last index. If you iterate through the indices 1 through [`endindex(s)`](@ref)
and index into `s`, the sequence of characters returned when errors aren't thrown is the sequence
of characters comprising the string `s`. Thus we have the identity that `length(s) <= endindex(s)`,
...
```jldoctest unicodestring
julia> for i = begindex(s):endindex(s)
           try
               println(s[i])
           catch
...
Some other useful functions include:
  ...
  * [`beginindex(str)`](@ref) gives the minimal (byte) index that can be used to index into `str` (always 1 for strings, not necessarily true for other containers).
  * [`endindex(str)`](@ref) gives the maximal (byte) index that can be used to index into `str`.
  ...
  * [`c, j = next(str,i)`](@ref next) returns next character at or after the index `i` and the next valid
    character index following that. With [`start`](@ref) and [`endindex`](@ref), can be used to iterate
