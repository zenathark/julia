...
for str in [astr, GenericString(astr)]
    ...
    @test_throws BoundsError findnext(equalto('ε'), str, nextind(str,endindex(str))+1)
    @test_throws BoundsError findnext(equalto('a'), str, nextind(str,endindex(str))+1)
end
...
for str in (u8str, GenericString(u8str))
    ...
    @test findnext(equalto('ε'), str, nextind(str,endindex(str))) == 0
    @test findnext(equalto('a'), str, nextind(str,endindex(str))) == 0
    @test_throws BoundsError findnext(equalto('ε'), str, nextind(str,endindex(str))+1)
    @test_throws BoundsError findnext(equalto('a'), str, nextind(str,endindex(str))+1)
end
...
for i = 1:endindex(astr)
    @test findnext(r"."s, astr, i) == i:i
end
for i = 1:endindex(u8str)
    if isvalid(u8str,i)
