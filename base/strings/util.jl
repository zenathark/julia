...
```
"""
chop(s::AbstractString) = SubString(s, start(s), prevind(s, endindex(s)))
chop(s::AbstractString, head::Integer, tail::Integer) =
    SubString(s, nextind(s, start(s), head), prevind(s, endindex(s), tail))
...
_lastbyteidx(s::AbstractVector{UInt8}) = endindex(s)
