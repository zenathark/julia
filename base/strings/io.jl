...
tostr_sizehint(x) = 0
tostr_sizehint(x::AbstractString) = endindex(x)
...
"""
escape_string(s::AbstractString, esc::AbstractString) = sprint(escape_string, s, esc, sizehint=endindex(s))
escape_string(s::AbstractString) = sprint(escape_string, s, "\"", sizehint=endindex(s))
...
[`escape_string`](@ref).
"""
unescape_string(s::AbstractString) = sprint(unescape_string, s, sizehint=endindex(s))
