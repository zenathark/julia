...
with the appropriate function calls (`endindex` or `size`). Replacement uses
...
function replace_ref_end_!(ex, withex)
    ...
    elseif isa(ex,Expr)
        if ex.head == :ref
            ...
            elseif nargs == 1
                # replace with endindex(S)
                ex.args[2], used_S = replace_ref_end_!(ex.args[2],:($endindex($S)))
            else
                n = 1
                J = endindex(ex.args)
                for j = 2:J
...
# size and endindex in replace_ref_end!) must be interpolated
