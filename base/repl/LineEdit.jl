...
function common_prefix(completions)
    ...
    while true
        for c in completions
            (i > endindex(c) || c[i] != cc) && return ret
        end
        ret = string(ret, cc)
        i >= endindex(c1) && return ret
        i = nexti
        cc, nexti = next(c1, i)
    end
end
...
function endindexline(buf, pos=position(buf))
...
function push_kill!(s::MIState, killed::String, concat = s.key_repeats > 0; rev=false)
    ...
    s.kill_idx = endindex(s.kill_ring)
    true
end
...
function edit_transpose_lines_up!(buf::IOBuffer, reg::Region)
    ...
    b = endindexline(buf, last(reg) - b2 + b1) # b2-b1 is the size of the removed line1
...
function edit_transpose_lines_down!(buf::IOBuffer, reg::Region)
    e1 = endindexline(buf, last(reg))
    e1 == buf.size && return false
    e2 = endindexline(buf, e1+1)
