...
function complete_path(path::AbstractString, pos; use_envpath=false)
    ...
    startpos = pos - endindex(prefix) + 1 - length(matchall(r" ", prefix))
    # The pos - endindex(prefix) + 1 is correct due to `endindex(prefix)-endindex(prefix)==0`,
    # hence we need to add one to get the first index. This is also correct when considering
    # pos, because pos is the `endindex` a larger string which `endswith(path)==true`.
...
function find_start_brace(s::AbstractString; c_start='(', c_end=')')
    ...
    return (startind:endindex(s), method_name_end)
end
...
function shell_completions(string, pos)
    ...
    elseif isexpr(arg, :incomplete) || isexpr(arg, :error)
        ...
        partial = scs[r]
        ret, range = completions(partial, endindex(partial))
